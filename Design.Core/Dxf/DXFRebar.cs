...
namespace Design.Core.Dxf
{
    public class DXFRebar
    {
        ...
        public static DxfLine[] TopSpanRFT(DxfModel model, int nSpans, double thickness, double[] Ln, Point2D[] startPointsTop, Point2D[] endPointsTop)
        {
            ...
            TopSpan[0] = new DxfLine(EntityColors.Blue, new Point2D(startPointsTop[0].X - 0.20, thickness - 0.07),
                new Point2D(endPointsTop[0].X - Math.Max(0.33 * Ln[0], 0.33 * Ln[1]) + 0.25, thickness - 0.07));
            model.Entities.Add(TopSpan[0]);

            for (int i = 1; i < TopSpan.Length - 2; i++)
            {
                TopSpan[i] = new DxfLine(EntityColors.Blue, new Point2D(startPointsTop[i].X + Math.Max(0.33 * Ln[i - 1], 0.33 * Ln[i]) - 0.25, thickness - 0.07)
                    , new Point2D(endPointsTop[i].X - Math.Max(0.33 * Ln[i], 0.33 * Ln[i + 1]) + 0.25, thickness - 0.07));
                model.Entities.Add(TopSpan[i]);
            }
            TopSpan[nSpans] = new DxfLine(EntityColors.Blue, new Point2D(startPointsTop[nSpans - 1].X + 0.33 * Ln[nSpans - 1] - 0.25, thickness - 0.07)
                , new Point2D(endPointsTop[nSpans - 1].X + 0.20, thickness - 0.07));
        ...
        public static void Legs(DxfModel model, int nSpans, double thickness, DxfLine[] BottomRFT, DxfLine[] TopSpan)
        {
            ...
            DxfLine legBot1 = new DxfLine(EntityColors.Blue, new Point2D(BottomRFT[0].Start.X, BottomRFT[0].Start.Y)
                , new Point2D(BottomRFT[0].Start.X, thickness - 0.15));
            ...
            DxfLine legTop2 = new DxfLine(EntityColors.Blue, new Point2D(TopSpan[nSpans].End.X, TopSpan[nSpans].End.Y)
                , new Point2D(TopSpan[nSpans].End.X, TopSpan[nSpans].End.Y - thickness + 0.15));
